...
@Component
public class Catalogo_ProductoDao {

    private static final Logger log = LoggerFactory.getLogger(Catalogo_ProductoDao.class);
    ...
    void init() {
        ...
        //insertar
        crear = new SimpleJdbcCall(jdbcTemplate).withProcedureName("SP_CREAR_CATALOGO_PRODUCTO");
        // Convert SYS_REFCURSOR to List
        obtener = new SimpleJdbcCall(jdbcTemplate)
                .withProcedureName("SP_GET_CATALOGO_PRODUCTO")
    ...
    //insertar
    public Optional crear(Catalogo_Producto obj){
    ...
    //actualizar
    public Optional actualizar(Catalogo_Producto obj){
    ...
    public Optional borrar(Long id){
        SqlParameterSource in = new MapSqlParameterSource().addValue("IN_CAT_PROD_ID", id);
        Optional result = Optional.empty();
    ...
    //obtener
    public List<Catalogo_Producto> obtener(Long usuario_id) {

        log.info("SP_GET_CATALOGO_PRODUCTO.obtener...");
